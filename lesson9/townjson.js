    ...
    for (let i = 0; i < towns.length; i++ ) {
      if (towns[i].name == 'Fish Haven' || towns[i].name == 'Preston' || towns[i].name == 'Soda Springs') {
        ...
        for (let t = 0; t < towns[i].events.length; t++) {
          if (myPath.includes("preston-10.html") && towns[i].name == 'Preston') {
            let event = document.createElement('p');
            event.textContent = towns[i].events[t];
            document.querySelector('div.events').appendChild(event);
          }
          if (myPath.includes("fishHaven-11.html") && towns[i].name == 'Fish Haven') {
            let event = document.createElement('p');
            event.textContent = towns[i].events[t];
            document.querySelector('div.events').appendChild(event);
          }
          if (myPath.includes("sodaSprings-11.html") && towns[i].name == 'Soda Springs') {
            let event = document.createElement('p');
            event.textContent = towns[i].events[t];
            document.querySelector('div.events').appendChild(event);
          }
